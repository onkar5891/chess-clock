...
class ChessClock extends React.Component {
  ...
  configurationRequired = (timerOptions: TimerOptions) => timerOptions.minutes === 0 && timerOptions.seconds === 0

  handleNewGame = (timerOptions: TimerOptions) => {
    this.whiteTimerManager = new TimerManager(timerOptions, "White", this.resolveWhiteTimer)
    this.blackTimerManager = new TimerManager(timerOptions, "Black", this.resolveBlackTimer)
    this.setState({
  ...
  handleWhiteTimerUpdates = () => {
    if (this.state.white.indicator === "finished" || this.state.black.indicator === "finished") {
        ...
        <Row className="centered-justification">
          ...
          <Col>
            <Row className="align-items-center">
              <Col xs={5} lg={5}>
                <Timer color="White" timerOptions={this.state.white.timerOptions} onTimerClick={this.handleWhiteTimerUpdates} />
              </Col>
              <Col><Configuration onNewGame={this.handleNewGame} /></Col>
              <Col xs={5} lg={5}>
                <Timer color="Black" timerOptions={this.state.black.timerOptions} onTimerClick={this.handleBlackTimerUpdates} />
              </Col>
            </Row>
          </Col>
          <Col xs={1} lg={1}></Col>
